...
$paket = db()->fetchOne(
  "SELECT * FROM paket_umrah WHERE id = ? AND status = 'aktif'",
  'i',
  [$paketId]
);
if (!$paket) {
...
if ($jadwalId) {
  $jadwal = db()->fetchOne(
    "SELECT * FROM jadwal WHERE id = ? AND paket_id = ? AND status = 'aktif'",
    'ii',
    [$jadwalId, $paketId]
  );
}
$jadwals = db()->fetchAll(
  "SELECT * FROM jadwal WHERE paket_id = ? AND status = 'aktif'
     AND tanggal_berangkat >= CURDATE() ORDER BY tanggal_berangkat ASC",
  'i',
  [$paketId]
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $selJadwal = $jId ? db()->fetchOne(
    "SELECT * FROM jadwal WHERE id = ? AND paket_id = ? AND status = 'aktif'",
    'ii',
    [$jId, $paketId]
  ) : null;
  $finalHarga = $selJadwal && $selJadwal['harga_khusus']
    ? $selJadwal['harga_khusus']
    : $paket['harga'];
  ...
  if (empty($errors)) {
    ...
    // --- FIX BUG #3: Transaksi + SELECT FOR UPDATE agar kuota aman ---
    $conn->begin_transaction();
    try {
      // Lock baris paket supaya tidak ada proses lain yang bisa update
      // sisa_kuota bersamaan (mencegah race condition / overbooking)
      $paketLock = db()->fetchOne(
        "SELECT sisa_kuota FROM paket_umrah WHERE id = ? FOR UPDATE",
        'i',
        [$paketId]
      );

      if (!$paketLock || $paketLock['sisa_kuota'] < $jQty) {
        $conn->rollback();
        $errors[] = 'Kuota tidak mencukupi untuk ' . $jQty . ' jamaah. Silakan pilih jumlah lebih kecil.';
      } else {
        ...
        // --- FIX BUG #1: Generate kode booking sequential dari AUTO_INCREMENT ---
        // Strategi: insert dulu dengan kode sementara, ambil ID yang
        // dijamin unik oleh DB, lalu update kode menjadi SAH-YYYY-XXXX.
        // Dengan cara ini tidak mungkin duplikat meski ada banyak request
        // bersamaan, karena AUTO_INCREMENT dikelola penuh oleh MySQL.

        $bookingId = db()->insert(
          "INSERT INTO booking
                        (kode_booking, nama_pemesan, email, telepon, alamat,
                         paket_id, jadwal_id, jumlah_jamaah, harga_per_orang,
                         total_harga, dp_amount, catatan, data_jamaah, status)
                     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'pending')",
          'sssssiiidddss',
          [
            'TEMP-' . time(),   // kode sementara, segera diganti
            $namaPemesan,
            $email,
            $telepon,
            $alamat,
            ...
            $finalDp,
            $catatan,
            json_encode($dataJamaah)
          ]
        );

        if (!$bookingId) {
          throw new RuntimeException('Insert booking gagal: ' . db()->getError());
        }

        // Sekarang kita punya ID unik dari AUTO_INCREMENT —
        // gunakan untuk membentuk kode booking yang rapi dan berurutan
        $kodeBooking = 'SAH-' . date('Y') . '-' . str_pad($bookingId, 4, '0', STR_PAD_LEFT);

        // Update kode dari TEMP ke kode final
        db()->execute(
          "UPDATE booking SET kode_booking = ? WHERE id = ?",
          'si',
          [$kodeBooking, $bookingId]
        );

        // Kurangi sisa_kuota — aman karena sudah di dalam transaksi
        $affected = db()->execute(
          "UPDATE paket_umrah SET sisa_kuota = sisa_kuota - ? WHERE id = ? AND sisa_kuota >= ?",
          'iii',
          [$jQty, $paketId, $jQty]
        );
        if (!$affected) {
          throw new RuntimeException('Kuota habis terkunci.');
        }

        if ($jId) {
          db()->execute(
            "UPDATE jadwal SET terisi = terisi + ? WHERE id = ?",
            'ii',
            [$jQty, $jId]
          );
        }
      ...
      $conn->rollback();
      $errors[] = $e->getMessage() === 'Kuota habis terkunci.'
        ? 'Kuota habis, silakan coba lagi.'
  ...
  <section style="padding:50px 0 80px">
    <div class="container">
      ...
      <form method="POST" action="" id="formBooking">
        <!-- FIX BUG #2: CSRF token field wajib ada -->
        <?= csrfField() ?>
            ...
            <div class="card-form">
              ...
              <div class="row g-3">
                <div class="col-md-6">
                  ...
                  <input type="text" name="nama_pemesan" class="form-control" placeholder="Nama sesuai KTP/Paspor"
                    value="<?= htmlspecialchars($old['nama_pemesan'] ?? '') ?>" required>
                </div>
                <div class="col-md-6">
